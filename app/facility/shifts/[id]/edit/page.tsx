...
export default function EditFacilityShiftPage() {
  ...
  if (!canEditShift(shift.data)) {
    return (
      <div className="space-y-4">
        <ButtonLink href={`/facility/shifts/${id}`} variant="ghost" className="px-0">
  ...
  async function save(values: ShiftFormValues) {
    try {
      ...
      router.push(`/facility/shifts/${id}`);
    } catch (error) {
    ...
    <div className="mx-auto max-w-2xl space-y-5">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-ink-muted">
          Update this day&apos;s hours, location, or notes.
        </p>
      </div>
