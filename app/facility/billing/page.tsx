...
import {
  downloadMyFacilityInvoicePdf,
  getMyFacilityInvoices,
} from "@/lib/api";
...
export default function FacilityBillingPage() {
  ...
  const invoices = useQuery({
    queryKey: ["my-facility-invoices", page, pageSize],
    queryFn: () => getMyFacilityInvoices(page, pageSize),
  });
    ...
    <div className="mx-auto max-w-2xl space-y-6 animate-rise">
      <div>
        ...
        <p className="mt-1 text-sm text-ink-muted">
          Invoices from OkayNow for completed facility shifts. Download a PDF to
          print or save. Pay by the due date (or contact the agency). If you hire
          a caregiver you met here for ongoing private care, report it below —
          the platform conversion fee in Terms applies.
        </p>
      </div>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              <Button
                ...
                onClick={async () => {
                  setBusyId(inv.id);
                  try {
                    await downloadMyFacilityInvoicePdf(
                      inv.id,
                      inv.invoiceNumber,
                    );
